    ...
    <div className="flex flex-wrap gap-1.5">
      {actions.map((action) => (
        <button
          ...
          className="rounded-lg bg-white px-2.5 py-1.5 text-xs font-medium text-violet-900 shadow-sm ring-1 ring-violet-200/80 hover:bg-violet-100 disabled:opacity-50"
        >
          {ai.loading === action ? (
...
export function EditorAiSidebar({ noteTitle, onClose, ai }: Props) {
  ...
  return (
    <aside
      className="flex h-[min(70vh,640px)] flex-col rounded-xl border border-violet-200/80 bg-gradient-to-b from-violet-50/90 to-white shadow-sm lg:sticky lg:top-4 lg:h-[calc(100vh-6rem)] lg:max-h-[calc(100vh-6rem)]"
    ...
    >
      <div className="flex items-center justify-between gap-2 border-b border-violet-100 px-3 py-2.5">
        ...
        <button
          type="button"
          onClick={onClose}
          className="rounded-md p-1.5 text-zinc-500 hover:bg-violet-100 hover:text-zinc-800"
          title="Close panel"
        >
      ...
      <div className="space-y-2 border-b border-violet-50 px-2 py-2">
        <div>
          <p className="mb-1 px-0.5 text-[10px] font-semibold uppercase tracking-wide text-zinc-500">
            Whole note
          </p>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3">
        ...
        {output !== null && (
          <div className="space-y-2">
            ...
            <pre className="whitespace-pre-wrap break-words rounded-lg border border-zinc-200 bg-zinc-50/80 p-3 text-xs leading-relaxed text-zinc-800">
              {output}
            </pre>
          </div>
        )}
