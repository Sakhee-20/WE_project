...
type Props = {
  ...
  /** Current text after "/", used to reset selection only when the filter changes. */
  query: string;
...
export const SlashCommandMenu = forwardRef<SlashCommandMenuHandle, Props>(
  function SlashCommandMenu({ items, query, command }, ref) {
    ...
    const shellClass = cn(
      "slash-command-menu-shell max-h-[min(320px,50vh)] w-[min(300px,calc(100vw-16px))] overflow-hidden rounded-xl border py-1 shadow-2xl",
    ...
    return (
      <div
        ...
        aria-activedescendant={`slash-cmd-${items[selected]?.id ?? ""}`}
      >
        <div className="slash-command-menu-scroll max-h-[min(308px,calc(50vh-12px))] overflow-y-auto overflow-x-hidden px-1">
          {items.map((item, index) => {
            const Icon = ICONS[item.id] ?? Heading2;
